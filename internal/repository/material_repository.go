...
type MaterialRepositoryInterface interface {
	Create(ctx context.Context, conn *pgx.Conn, data *dto.CreateMaterialDTO) (*dto.MaterialDTO, error)
	GetByID(ctx context.Context, conn *pgx.Conn, id int) (*dto.MaterialDTO, error)
	GetAll(ctx context.Context, conn *pgx.Conn) ([]*dto.MaterialDTO, error)
	Update(ctx context.Context, conn *pgx.Conn, id int, data *dto.UpdateMaterialDTO) (*dto.MaterialDTO, error)
...
func (r *MaterialRepository) GetByID(ctx context.Context, conn *pgx.Conn, id int) (*dto.MaterialDTO, error) {
	query := `SELECT material_id, source, extension, size FROM material WHERE material_id = $1`
	var material dto.MaterialDTO
	err := conn.QueryRow(ctx, query, id).Scan(&material.MaterialID, &material.Source, &material.Extension, &material.Size)
	if err != nil {
		return nil, err
	}
	return &material, nil
...
func (r *MaterialRepository) GetAll(ctx context.Context, conn *pgx.Conn) ([]*dto.MaterialDTO, error) {
