...
type LessonHomeworksRepositoryInterface interface {
	...
	GetAll(ctx context.Context, conn *pgx.Conn) ([]*dto.LessonHomeworksDTO, error)
	Delete(ctx context.Context, conn *pgx.Conn, lessonID, homeworkID int) error
}
...
func (r *LessonHomeworksRepository) Create(ctx context.Context, conn *pgx.Conn, data *dto.CreateLessonHomeworksDTO) (*dto.LessonHomeworksDTO, error) {
	...
	err := conn.QueryRow(ctx, query, data.LessonID, data.HomeworkID).Scan(&relation.LessonID, &relation.HomeworkID)
	if err != nil {
		return nil, err
	}
	return &relation, nil
...
func (r *LessonHomeworksRepository) GetByID(ctx context.Context, conn *pgx.Conn, lessonID, homeworkID int) (*dto.LessonHomeworksDTO, error) {
	query := `SELECT lesson_id, homework_id FROM lesson_homeworks WHERE lesson_id = $1 AND homework_id = $2`
	var relation dto.LessonHomeworksDTO
	err := conn.QueryRow(ctx, query, lessonID, homeworkID).Scan(&relation.LessonID, &relation.HomeworkID)
	if err != nil {
		return nil, err
	}
	return &relation, nil
...
func (r *LessonHomeworksRepository) GetAll(ctx context.Context, conn *pgx.Conn) ([]*dto.LessonHomeworksDTO, error) {
...
func (r *LessonHomeworksRepository) Delete(ctx context.Context, conn *pgx.Conn, lessonID, homeworkID int) error {
	query := `DELETE FROM lesson_homeworks WHERE lesson_id = $1 AND homework_id = $2`
	_, err := conn.Exec(ctx, query, lessonID, homeworkID)
	return err
}
