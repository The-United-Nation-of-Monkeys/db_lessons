...
type TransactionsCoursesRepositoryInterface interface {
	...
	GetByID(ctx context.Context, tx pgx.Tx, transactionID, courseID int) (*dto.TransactionsCoursesDTO, error)
...
func (r *TransactionsCoursesRepository) GetByID(ctx context.Context, tx pgx.Tx, transactionID, courseID int) (*dto.TransactionsCoursesDTO, error) {
	query := `SELECT transaction_id, course_id FROM transactions_courses WHERE transaction_id = $1 AND course_id = $2`
	var relation dto.TransactionsCoursesDTO
	err := tx.QueryRow(ctx, query, transactionID, courseID).Scan(&relation.TransactionID, &relation.CourseID)
	if err != nil {
		return nil, err
	}
	return &relation, nil
